...
def load_multi_query_prompt():
    try:
        multi_query_prompt_path = get_abs_path(prompts_conf["multi_query_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_multi_query_prompt]在yaml配置项中没有multi_query_prompt_path配置项")
        raise e

    try:
        return open(multi_query_prompt_path, "r", encoding="utf-8").read()
    except Exception as e:
        logger.error(f"[load_multi_query_prompt]解析多查询提示词出错，{str(e)}")
        raise e
